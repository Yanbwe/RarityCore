...
public class SimpleConfigValidator {
     ...
     */
    public static ItemDataMatchRule parseRule(JsonObject config) {
     ...
     * @return 条件列表
     */
    private static List<ItemDataCondition> parseConditions(com.google.gson.JsonArray conditionsArray) {
        List<ItemDataCondition> conditions = new ArrayList<>();
        
        for (int i = 0; i < conditionsArray.size(); i++) {
            try {
                JsonObject conditionObj = conditionsArray.get(i).getAsJsonObject();
                ItemDataCondition condition = parseCondition(conditionObj);
                if (condition != null) {
     ...
     */
    private static ItemDataCondition parseCondition(JsonObject conditionObj) {
     ...
     * 解析等值条件
     */
    private static ItemDataCondition parseEqualsCondition(JsonObject conditionObj,
     ...
     * 解析范围条件
     */
    private static ItemDataCondition parseRangeCondition(JsonObject conditionObj,
                                                    String path, String description) {
     ...
     * 解析包含条件
     */
    private static ItemDataCondition parseContainsCondition(JsonObject conditionObj,
                                                       String path, String description) {
