...
public class StarDisplayConfigManager {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(StarDisplayConfigManager.class);
                    ...
                    if (customObj.has("strings")) {
                        ...
                        for (String key : stringsObj.keySet()) {
                            try {
                                ...
                                LOGGER.warn("Invalid rarity key in custom star strings: {}", key);
                            }
                        }
                    }
                    ...
                    if (customObj.has("specialRarityTexts")) {
                        ...
                        for (String key : specialTextsObj.keySet()) {
                            try {
                                ...
                                LOGGER.warn("Invalid rarity key in special rarity texts: {}", key);
                            }
                        }
                    }
                }
                
                LOGGER.info("Loaded star display config: enabled={}, mode={}, repeatChar='{}', customStrings={}, specialRarityTexts={}", 
                    enableStarDisplay, starMode, repeatCharacter, customStarStrings.size(), customSpecialRarityTexts.size());
            } else {
                // 如果没有starDisplay配置,使用默认值
                resetToDefaults();
                LOGGER.info("No star display config found, using defaults");
            }
        } catch (Exception e) {
            LOGGER.error("Error loading star display config, using defaults: {}", e.getMessage());
            // 出错时使用默认值
            resetToDefaults();
        } finally {
