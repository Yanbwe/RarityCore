...
public class ConfigFileUtils {
    ...
    public static boolean ensureDirectoryExists(Path directoryPath, String operationName) {
        try {
            if (!Files.exists(directoryPath)) {
                ...
                LOGGER.info("Created directory for {}: {}", operationName, directoryPath);
            }
            return true;
        } catch (IOException e) {
            LOGGER.error("Failed to create directory for {}: {}", operationName, directoryPath, e);
            return false;
        }
    }
    ...
    public static JsonObject readJsonConfig(Path configFile, String operationName) {
        ...
        try (BufferedReader reader = Files.newBufferedReader(configFile)) {
            ...
            LOGGER.warn("Failed to parse {} config file, will recreate: {}", operationName, configFile, e);
            return new JsonObject();
        }
    }
    ...
    public static boolean writeJsonConfig(Path configFile, JsonObject jsonObject, String operationName) {
        try (FileWriter writer = new FileWriter(configFile.toFile())) {
            ...
            LOGGER.info("Successfully wrote {} config file: {}", operationName, configFile);
            return true;
        } catch (IOException e) {
            LOGGER.error("Failed to write {} config file: {}", operationName, configFile, e);
            return false;
        }
    }
    ...
    public static boolean updateJsonConfig(Path configFile, JsonUpdater updater, String operationName) {
        ...
        try {
            ...
            LOGGER.error("Error updating {} config: {}", operationName, e.getMessage(), e);
            return false;
        }
