...
### Fixed
- Fixed the Iron's Spells adapter being **completely non-functional**, so spell level no longer affected rarity: `IronSpellsAdapter` cached its reflection target under a wrong class name — `SpellContainer` actually lives in `io.redspace.ironsspellbooks.capabilities.magic`, but the code looked for `api.spells.SpellContainer`. `Class.forName` therefore threw, and the adapter marked itself as "not loaded" and degraded silently
  - Consequence: the Iron's Spells branch never matched in the priority chain, so every spell scroll/book fell through to the static datapack mapping and its rarity **no longer varied with spell level** (the same item showed one rarity at every level)
  - The only trace was a `Failed to cache Iron's Spells method references` WARN, still followed by an `Iron's Spells compatibility adapter initialized` INFO line, which made it easy to misread as healthy. That log is now accurate, and a new `isFunctional()` distinguishes "mod not installed" from "mod present but adapter unusable"
  - Class resolution now tries several candidates (implementation first, interface as fallback); the component value's actual type is the `ISpellContainer` interface (`SpellContainer.CODEC` is declared as `Codec<ISpellContainer>`), so resolving either one is sufficient
- Fixed spell level being read from a **hard-coded slot 0**: the previous implementation called `getSpellAtIndex(0)`, which threw `IllegalStateException` when the spell did not sit in slot 0; the outer catch turned that into a single WARN and the item silently lost its Iron's Spells rarity. It now prefers `getActiveSpells()` (which returns only **non-empty** slots) and reads `SpellSlot.getLevel()`, so the slot index no longer matters; `getSpellAtIndex(0)` remains only as a fallback
